...
def get_picture_tensors(root_directory, 
    ...
    if shuffle_directories:
        random.shuffle(subdirectories)

    # créer des listes vides
    train_images, val_images, test_images, train_labels, val_labels, test_labels, label_to_directory = [[] for _ in range(7)]
    ...
    while current_amount_of_classes < n_classes and current_index < max_index:
        ...
        if len(jpeg_files) < required_images_per_cat:
            ...
            label_to_directory.append(subdirectory)
    ...
    info = {'n_classes' : n_classes_to_use,
             'label_to_directory' : label_to_directory}

    return train_images, val_images, test_images, train_labels, val_labels, test_labels, info
